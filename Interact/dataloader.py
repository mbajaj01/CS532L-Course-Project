...
class DataLoader:
	...
	def processParams(self):
		self.unique_img_train = np.array(self.params['unique_img_train'])
		self.unique_img_test = np.array(self.params['unique_img_test'])
		self.unique_img_val = np.array(self.params['unique_img_val'])
	...
	def getBatch(self, indexes, dataset):
		...
		if self.isRightAligned:
			batch['questions'] = self.questions_aligned[dataset][indexes,:,self.questions_aligned[dataset].shape[2] - batch_max_ques_length:]
		else:
		...
		batch['image_pos'] = self.image_pos[dataset][indexes]
	...
	def getQBatch(self, indexes, dataset):
		...
		if self.isRightAligned:
			batch['answers'][:,1:,:] = self.answers_aligned[dataset][indexes,:,self.answers_aligned[dataset].shape[2] - batch_max_answer_length:]
		else:
		...
		batch['image_pos'] = self.image_pos[dataset][indexes]
		if dataset == 'test':
			batch['image_id'] = self.unique_img_test[indexes]
	...
	def rightAlignAll(self):
		...
		self.questions_aligned = {}
		self.answers_aligned = {}
		for dataset in ['train', 'test', 'val']:
			self.questions_aligned[dataset] = self.rightAlign(self.questions[dataset], self.questions_length[dataset])
			self.answers_aligned[dataset] = self.rightAlign(self.answers[dataset], self.answers_length[dataset])
