...
#             param.requires_grad = False
        ...
        self.linear_vocab_layer = nn.Linear(params['vocab_size'],params['embed_size'])
        self.linear_vocab_layer = nn.Linear(params['vocab_size'],params['embed_size'])
...
#         self.ques_embedding_layer = ABot.EmbeddingLayer(embedding_weights)
    ...
    def forward(self, question_batch, question_batch_len, answer_batch, answer_batch_len, batch_size, image_tensor):
        batch_mode = True
        sort_index = 0 if self.params['batch_first'] else 1

        question_batch_embedding = self.linear_vocab_layer(question_batch)
        answer_batch_embedding = self.linear_vocab_layer(answer_batch)
...
#         answer_batch_embedding = self.ans_embedding_layer(answer_batch)
        ...
        outImages = self.img_embed_layer(image_tensor)
        #print(out.size())
...
def LoadDiscriminator(params):
    ...
    checkpoint = torch.load('outputs/discr_bad_5')
...
def get_discriminator_loss(discr, sampledAnswers,ans_sample_len, sampledQuestions, ques_sample_len, image_tensor, params, batch_size=None):
...
#         print("D: loss: R: ",d_real_loss.data[0],"F: ",d_fake_loss.data[0], "C: ", d_train_loss.data[0])
    ...
    g_fake_result = discr(sampledQuestions, ques_sample_len, sampledAnswers, ans_sample_len, batch_size, image_tensor).squeeze()
