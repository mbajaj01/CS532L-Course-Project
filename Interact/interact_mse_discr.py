...
import QBot_Encoder
import QBot_Decoder
import train_discr_gen_bad_api as discriminator_file
...
discriminator, discriminator_optimizer = discriminator_file.LoadDiscriminator(params)
...
def InteractTrain(ABatch, QBatch, AEncoder, ADecoder, QEncoder, QDecoder, ABot_embedding_layer, QBot_embedding_layer, sampler, discriminator, params, ABot_optimizer, QBot_optimizer, discriminator_optimizer, criterion, numSL, batch_mode=True, volatile=False, num_dialogs=10, onlyForward=False, train_discriminator=False):
	average_loss = 0.0
...
#	aa
	...
	for dialog_num in range(num_dialogs):
		...
		if dialog_num < numSL:
			if not onlyForward:
				answer_batch = QBatch['answers'][:,dialog_num,:].astype(np.int)
				question_input_batch = QBatch['questions_input'][:,dialog_num,:].astype(np.int)
				question_output_batch = QBatch['questions_output'][:,dialog_num,:].astype(np.int)
				question_batch_len = QBatch['questions_length'][:,dialog_num].astype(np.int)
				answer_batch_len = QBatch['answers_length'][:,dialog_num].astype(np.int)
				history_batch = QBatch['history'][:,:dialog_num+1,:].astype(np.int)
				history_batch_len = QBatch['history_length'][:,:dialog_num+1].astype(np.int)
				...
				answer_batch_embedding = QBot_embedding_layer(answer_batch_tensor)
				fact_batch_embedding = QBot_embedding_layer(history_batch_tensor.view(-1, history_batch_tensor.size(2)))
				...
				question_input_batch_embedding = QBot_embedding_layer(question_input_batch_tensor)
				probabilities_question_batch, true_question_decoder_batch_hidden, _ = QDecoder(question_input_batch_embedding, question_decoder_batch_hidden, batch_mode=batch_mode)
				...
				total_loss_Q = loss_mse + loss_masked
				...
				answer_input_batch = ABatch['answers_input'][:,dialog_num,:].astype(np.int)
				answer_output_batch = ABatch['answers_output'][:,dialog_num,:].astype(np.int)
				question_batch_len = ABatch['questions_length'][:,dialog_num].astype(np.int)
				answer_batch_len = ABatch['answers_length'][:,dialog_num].astype(np.int)
				history_batch = ABatch['history'][:,:dialog_num+1,:].astype(np.int)
				history_batch_len = ABatch['history_length'][:,:dialog_num+1].astype(np.int)
				#image_batch = ABatch['images'].astype(np.float)
				...
				question_batch_embedding = ABot_embedding_layer(question_batch_tensor)
				fact_batch_embedding = ABot_embedding_layer(history_batch_tensor.view(-1, history_batch_tensor.size(2)))
				history_batch_length_tensor = history_batch_length_tensor.view(-1,)
...
#			print ([[params['ind2word'][x] for x in ABot_sampled_words[i].data.cpu().numpy() if x != 0] for i in range(batch_size)])
			...
			if dialog_num == num_dialogs-1 and onlyForward:
				print ([params['ind2word'][x] for x in QBot_sampled_words[0].data.cpu().numpy() if x != 0])
				print ([params['ind2word'][x] for x in ABot_sampled_words[0].data.cpu().numpy() if x != 0])
			if not train_discriminator:
				...
				#if dialog_num == num_dialogs-1:
				loss_mse = criterion['MSELoss'](image_output.squeeze(0), image_batch_tensor)
				discr_output = discriminator_file.get_discriminator_loss(discriminator, ABot_one_hot_samples, ABot_generated_answer_length_tensor, QBot_one_hot, QBot_generated_question_length_tensor, image_batch_tensor, params, batch_size=batch_size)
				y_real = Variable(torch.ones(batch_size))
				if params['USE_CUDA']:
					y_real = y_real.cuda(gpu)
				loss_discr = criterion['BCELoss'](discr_output, y_real)
				if dialog_num == num_dialogs-1:
					total_loss = 0.01*loss_mse + loss_discr
					if not onlyForward:
						QBot_optimizer.zero_grad()
						ABot_optimizer.zero_grad()
						total_loss.backward()
