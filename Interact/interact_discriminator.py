...
discriminator, discriminator_optimizer = LoadDiscriminator(params)
...
def train(data, params, AEncoder, ADecoder, QEncoder, QDecoder, ABot_embedding_layer, QBot_embedding_layer, sampler, discriminator, criterion, ABot_optimizer, QBot_optimizer, discriminator_optimizer, current_epoch=0):
	...
	for epoch in range(params['epochs']):
		...
		for batch_num in range(number_of_batches):
			...
			loss = InteractTrain(ABatch, QBatch, AEncoder, ADecoder, QEncoder, QDecoder, ABot_embedding_layer, QBot_embedding_layer, sampler, discriminator, params, ABot_optimizer, QBot_optimizer, discriminator_optimizer, criterion, numSL, batch_mode=True, volatile=True, onlyForward=True)
			tokens += loss[1]
			avgloss += loss[0]
...
def InteractTrain(ABatch, QBatch, AEncoder, ADecoder, QEncoder, QDecoder, ABot_embedding_layer, QBot_embedding_layer, sampler, discriminator, params, ABot_optimizer, QBot_optimizer, discriminator_optimizer, criterion, numSL, batch_mode=False, volatile=False, num_dialogs=10, onlyForward=False):
	average_loss = 0.0
	token_count = 0.0
	...
	for dialog_num in range(numSL, num_dialogs):
		...
		if dialog_num == num_dialogs - 1:
			total_loss = loss_mse + 100*loss_discr
			if not onlyForward:
				total_loss.backward()
				QBot_optimizer.step()
				ABot_optimizer.step()
