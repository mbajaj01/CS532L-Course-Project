...
class QBotDecoder(nn.Module):
    def __init__(self, params):
        super(QBotDecoder, self).__init__()
        self.params = params
        self.question_decoder = QBot.AnswerDecoder(self.params['embed_size'], self.params['hidden_dim'], self.params['vocab_size'], num_layers=self.params['num_layers'], rnn=self.params['rnn_type'], batch_first=self.params['batch_first'])
        self.image_decoder = QBot.ImageDecoder(self.params['hidden_dim']*2, self.params['vgg_out'])
    ...
    def forward(self, answer_input_embedding, answer_decoder_batch_hidden, batch_mode=True):
        ...
        probabilities_answer_batch, answer_decoder_batch_hidden = self.question_decoder(answer_input_embedding, answer_decoder_batch_hidden, isPacked=False)
        if not self.params['batch_first']:
            probabilities_answer_batch = probabilities_answer_batch.transpose(0,1).contiguous()
        # if self.params['rnn_type'] == 'LSTM':
        #     answer_hidden_embedding = F.tanh(self.linear(answer_decoder_batch_hidden[0].narrow(0,self.params['num_layers']-1,1)))
        # else:
        #     answer_hidden_embedding = F.tanh(self.linear(answer_decoder_batch_hidden.narrow(0,self.params['num_layers']-1,1)))
        return probabilities_answer_batch, answer_decoder_batch_hidden

    def beamSearch(self, answer_input_tokens, answer_decoder_batch_hidden, embedding_layer):
        batch_size = answer_input_tokens.size(0)
        ...
        output_probabilities, answer_decoder_batch_hidden = self.question_decoder(input_tokens_embedding, answer_decoder_batch_hidden, isPacked=False)
        if self.params['rnn_type'] == 'LSTM':
        ...
        for current_index in range(1, sequence_length):
            ...
            current_output_probabilities, beam_hidden_state = self.question_decoder(current_input_words_embeddings, beam_hidden_state, isPacked=False)
